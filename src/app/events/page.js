...
export const metadata = {
  title: "Events",
  description:
    "Stay updated on the latest events, webinars, and conferences from AVGC FramesShift.",
  alternates: {
    canonical: "https://avgcframes.com/events",
  },
};

export default async function EventsPage() {
  const categoryName = "Events";
  const data = await getLatestCategory("EVENTS");
  ...
  const jsonLd = {
    ...
    itemListElement: articles.slice(0, 10).map((item, index) => ({
      "@type": "ListItem",
      position: index + 1,
      url: `https://avgcframes.com/events/${item.slug}`,
      name: item.title,
    })),
  };
      ...
      <>
        ...
        <section className="max-w-7xl mx-auto px-4 py-32 flex flex-col items-center justify-center min-h-[50vh] font-sans">
          ...
          <p className="text-lg text-gray-600">
            No upcoming {categoryName.toLowerCase()} are currently listed.
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-10 gap-y-12 text-black">
          ...
          {articles.length > 0 && (
            <article className="flex flex-col group">
              <Link href={`/events/${articles[0]?.slug}`}>

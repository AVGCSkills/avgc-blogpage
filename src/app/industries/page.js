...
export const metadata = {
  title: "Industries",
  description:
    "Explore the latest insights, trends, and developments across various industries from AVGC FramesShift.",
  alternates: {
    canonical: "https://avgcframes.com/industries",
  },
};

export default async function IndustriesPage() {
  const categoryName = "Industries";
  const data = await getLatestCategory("INDUSTRIES");
  ...
  const jsonLd = {
    ...
    itemListElement: articles.slice(0, 10).map((item, index) => ({
      "@type": "ListItem",
      position: index + 1,
      url: `https://avgcframes.com/industries/${item.slug}`,
      name: item.title,
    })),
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-10 gap-y-12 text-black">
          ...
          {articles.length > 0 && (
            <article className="flex flex-col group">
              <Link href={`/industries/${articles[0]?.slug}`}>
